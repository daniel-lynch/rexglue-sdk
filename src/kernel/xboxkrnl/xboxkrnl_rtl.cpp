...
// PPC_FUNCs (not typed _entry shims) and registered by name.
//
...
extern "C" PPC_FUNC(__imp__RtlCaptureContext) {
  ...
  for (uint32_t i = 0; i < 18; ++i)
    PPC_STORE_U64(buf + i * 8, fpr[i].u64);

  // r1 (saved stack pointer) -> +0x090
  PPC_STORE_U64(buf + 144, ctx.r1.u64);

  // r13..r31 -> +0x098 (contiguous in PPCContext)
  const auto* gpr = &ctx.r13;
  for (uint32_t i = 0; i < 19; ++i)
    PPC_STORE_U64(buf + 152 + i * 8, gpr[i].u64);
  ...
  PPC_STORE_U32(buf + 304, cr);

  // LR (resume address) -> +0x134
  PPC_STORE_U32(buf + 308, static_cast<uint32_t>(ctx.lr));

  // mode flag -> +0x138: 0 selects the plain restore path in the guest routine
  PPC_STORE_U32(buf + 312, 0);

  // v64..v127 -> +0x140, byte-reversed to match lvx128/stvx128 ordering
  const auto* vr = &ctx.v64;
  for (uint32_t i = 0; i < 64; ++i) {
    uint8_t* dst = PPC_RAW_ADDR(buf + 320 + i * 16);
    for (uint32_t b = 0; b < 16; ++b)
      dst[b] = vr[i].u8[15 - b];
  }
}
static rex::detail::PPCFuncRegistrar _ppc_reg___imp__RtlCaptureContext(
...
extern "C" PPC_FUNC(__imp__RtlRestoreContext) {
  const uint32_t buf = ctx.r3.u32;
  if (!buf) {
  ...
  auto* fpr = &ctx.f14;
  for (uint32_t i = 0; i < 18; ++i)
    fpr[i].u64 = PPC_LOAD_U64(buf + i * 8);

  ctx.r1.u64 = PPC_LOAD_U64(buf + 144);

  auto* gpr = &ctx.r13;
  for (uint32_t i = 0; i < 19; ++i)
    gpr[i].u64 = PPC_LOAD_U64(buf + 152 + i * 8);

  const uint32_t cr = PPC_LOAD_U32(buf + 304);
  ...
  ctx.lr = PPC_LOAD_U32(buf + 308);

  auto* vr = &ctx.v64;
  for (uint32_t i = 0; i < 64; ++i) {
    const uint8_t* src = PPC_RAW_ADDR(buf + 320 + i * 16);
    for (uint32_t b = 0; b < 16; ++b)
      vr[i].u8[b] = src[15 - b];
  }

  // Transfer control to the restored LR with the restored r1.
  if (ctx.kernel_state && ctx.kernel_state->function_dispatcher()) {
    ::PPCFunc* fn = ctx.kernel_state->function_dispatcher()->GetFunction(static_cast<uint32_t>(ctx.lr));
    if (fn) {
      fn(ctx, base);
      return;
    }
...
static rex::detail::PPCFuncRegistrar _ppc_reg___imp__RtlRestoreContext(
...
extern "C" PPC_FUNC(__imp__RtlUnwind) {
  (void)base;
  ctx.r3.u64 = ctx.r6.u64;
}
static rex::detail::PPCFuncRegistrar _ppc_reg___imp__RtlUnwind("__imp__RtlUnwind",
                                                               &__imp__RtlUnwind);
